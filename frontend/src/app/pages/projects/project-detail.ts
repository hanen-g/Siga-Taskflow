...
import type { TaskDeadlinePredictionResponse } from '../../models/task-deadline-prediction.model';
...
export class ProjectDetailPage implements OnInit, OnDestroy {
  ...
  activeTab: 'tasks' | 'files' | 'chat' = 'tasks';
  chatUnreadCount = 0;
  ...
  deadlinePredictionDialogVisible = false;
  deadlinePredictionLoading = false;
  deadlinePredictionResult: TaskDeadlinePredictionResponse | null = null;
  ...
  /** Tasks returned with the project payload (same scope as completion %). */
  projectTotalTaskCount(project: Project | null): number {
    return project?.tasks?.length ?? 0;
  }

  projectTotalTaskCountLabel(project: Project | null): string {
    const n = this.projectTotalTaskCount(project);
  ...
  ngOnInit() {
    ...
    if (tab === 'chat' && this.canUseProjectChat()) {
      this.activeTab = 'chat';
  ...
  switchTab(tab: 'tasks' | 'files' | 'chat'): void {
    this.activeTab = tab;
    this.searchText = '';
    if (tab === 'chat') {
      this.chatUnreadCount = 0;
      // Chat panel is created by *ngIf after this tick; loading runs in its ngAfterViewInit.
    }
  }

  canUseProjectChat(): boolean {
    return this.isAdminRole() || this.isClientPortal();
  }

  private refreshChatUnreadCount(): void {
    if (!this.projectId || !this.canUseProjectChat()) {
      return;
    }
    this.projectChatService.getUnreadCount(this.projectId).subscribe({
      next: (response) => {
        this.chatUnreadCount = response.count ?? 0;
      },
      error: () => {
        this.chatUnreadCount = 0;
      }
  ...
  handleProjectAction(action: ProjectAction, project: Project | null, event: Event): void {
    ...
    if (action === 'add-task') {
      ...
      return;
    } else if (project.archived && action !== 'archive') {
      return;
    }
  ...
  searchCollaborators(event: AutoCompleteCompleteEvent): void {
    const q = event.query ?? '';
    this.userService.getCollabEmail(q).subscribe({
      next: (emails) => {
  ...
  closeTaskDialog(): void {
    ...
    this.assigneeCandidatesLoading = false;
    this.assigneeSuggestionsPopupVisible = false;
    this.closeDeadlinePredictionDialog();
    this.assigneeLoadSeq++;
  }

  canPredictNewTaskDeadline(): boolean {
    return (
      !!this.newTask.title?.trim() &&
      this.newTask.priority != null &&
      !!this.newTask.collaboratorEmail?.trim()
    );
  }

  predictNewTaskDeadline(project: Project): void {
    if (!project.id || !this.canPredictNewTaskDeadline() || this.deadlinePredictionLoading) {
      return;
    }
    this.deadlinePredictionLoading = true;
    this.deadlinePredictionResult = null;
    this.taskService
      .predictDeadline({
        ...
        collaboratorEmail: this.newTask.collaboratorEmail!.trim()
      })
      .subscribe({
        next: (result) => {
          this.deadlinePredictionLoading = false;
          this.deadlinePredictionResult = result;
          this.deadlinePredictionDialogVisible = true;
        },
        error: (err) => {
          this.deadlinePredictionLoading = false;
          const detail =
            err?.error?.error ??
            err?.error?.message ??
            'Could not predict a deadline. Make sure Ollama is running, then try again.';
          this.messageService.add({
            severity: 'error',
  ...
  applyPredictedDeadline(): void {
    const predicted = this.deadlinePredictionResult?.predictedDeadline;
    if (!predicted) {
      return;
    }
    this.newTask.deadline = this.toDateTimeLocalValue(predicted);
    this.closeDeadlinePredictionDialog();
  }

  closeDeadlinePredictionDialog(): void {
    this.deadlinePredictionDialogVisible = false;
    this.deadlinePredictionResult = null;
  }
  ...
  private confirmSetPaused(project: Project, paused: boolean, event: Event): void {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.projectService.setProjectLifecycle(project.id, { status: paused ? 'PAUSED' : 'IN_PROGRESS' }).subscribe({
          next: () => {
            this.loadProject(project.id);
  ...
  private confirmSetDelivered(project: Project, delivered: boolean, event: Event): void {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.projectService.setProjectLifecycle(project.id, { status: delivered ? 'COMPLETED' : 'IN_PROGRESS' }).subscribe({
          next: () => {
            this.loadProject(project.id);
